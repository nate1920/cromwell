...
object DatabaseTestKit extends StrictLogging {
  ...
  private def getDatabaseSystemSettings(networkDatabaseSystem: NetworkDatabaseSystem): DatabaseSystemSettings = {
    networkDatabaseSystem match {
      // The below list of docker tags should be synced with the tags under "BUILD_TYPE=sbt" in .travis.yml
      case MariadbDatabaseSystem => DatabaseSystemSettings("MARIADB", 23306, "5.5")
      case MariadbLatestDatabaseSystem => DatabaseSystemSettings("MARIADB_LATEST", 33306, "latest")
      case MysqlDatabaseSystem => DatabaseSystemSettings("MYSQL", 3306, "5.6")
      case MysqlLatestDatabaseSystem => DatabaseSystemSettings("MYSQL_LATEST", 13306, "latest")
      case PostgresqlDatabaseSystem => DatabaseSystemSettings("POSTGRESQL", 5432, "9.6")
      case PostgresqlLatestDatabaseSystem => DatabaseSystemSettings("POSTGRESQL_LATEST", 15432, "latest")
      // The list above of docker tags should be synced with the tags under "BUILD_TYPE=sbt" in .travis.yml
    }
  }
  ...
  private def getConfig(networkDatabaseSystem: NetworkDatabaseSystem): Config = {
    ...
    logger.info(
      s"""|The schema will initialized be when the docker container starts. If needed reset the schema using:
